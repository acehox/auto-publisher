...
- **`Channels.add` is register-or-unpause, and the unpause branch is plan-gated.** It is what holds the hot path's invariant: the publish path has no plan check, so unpausing a filter-paused row with its filters intact hands a free guild working Premium filtering — permanently, since the nightly backstop keys on channel count and the subscription reconcile on transitions, so an under-cap guild is never corrected. Filters + not Premium → `400 FILTERS_PREMIUM` carrying `{ filterCount }` (`HttpError.details`, forwarded by `sendErrorResponse`); `clearFilters: true` on the enable body is the admin's consent to drop the rule, clearing Postgres and the allowlist together. In the service, not the routes, so the dashboard and `/ap enable` cannot diverge. The cap check stays **first** — an over-cap guild hears about the cap before the rule. Both clients offer the same three-way choice (upgrade / remove the rule and enable / leave it off) rather than a flat refusal: the Filters surface that would let an admin clear the rule by hand is itself Premium-locked, so a bare error strands them. The dashboard asks before the permission guide (`channelLimit !== 0` **is** the free plan) and keeps the `FILTERS_PREMIUM` branch as the authority for a stale list; the bot asks on the rejection and retries with consent. **`hasSavedSetup` is only `pausedAt != null`** and says nothing about the cause, so filter copy reads `filters.length` — off the flag it told a channel paused purely for the cap that it held a rule, and made the paused strip quote a channel limit to a guild under it.
- **Disabling deletes the row, so a non-empty rule is confirmed first** — on both surfaces, Premium guilds included. The dashboard's Fix-dialog `Remove` shares the confirm; it is the same deletion.
...
- **Private-channel obfuscation: the dashboard list is a UNION, registration is not.** Since Discord's 16 Nov 2026 change, a channel the bot cannot view is **omitted entirely** from `GET /guilds/{id}/channels` — no row, no flag (the gateway still dispatches it, obfuscated with `name: "___hidden___"` and `flags & CHANNEL_OBFUSCATED`, but the REST list simply drops it). Publishing is unaffected: `ViewChannel` is in `PUBLISH_PERMISSION_FLAGS`, so no `MESSAGE_CREATE` ever arrives for such a channel. What breaks is the dashboard read: a *registered* channel goes hidden precisely because ViewChannel was revoked — the commonest way a channel breaks — and it would silently vanish from the payload while its row, filters and pause state stayed in Postgres, taking the Fix dialog with it. `Channels.getChannelsForDashboard` (`services/channels/dashboard.ts`, the sole caller being `GET /api/guild/:guildId`) therefore unions the REST list with `Guilds.getChannelRecords`: both inputs are durable, so no cache decides membership. Row-only entries get `canPublish: false` with no publish-state read — absent from REST *is* the proof — and are appended after the REST-ordered ones (no `position` to honour). `Channels.add` / `Guilds.migrate` keep the REST-only guard **deliberately unchanged**: a channel Discord will not tell us about is one nobody should be able to newly register, and hidden channels are only ever reachable through an existing row. A hidden channel nobody enabled is undiscoverable — accepted. **`GuildChannel.name` is `string | null`, never a `hidden` boolean**: absent-from-REST also covers a channel deleted while the bot was offline (`channelDelete` only fires while online, `pruneStaleChannels` runs on re-join only), and we cannot tell the two apart — a flag would assert what we don't know, while a null name states exactly what we do for both causes, and the nullable type makes the compiler enumerate every render site. `channelLabel(null)` → `'Hidden channel'`; `channelIsHidden()` sits beside `channelIsBroken()`. Rendered **red with `Fix`, the same bucket as any broken channel** — so `needsFixingCount`, the misconfigured strip and the sidebar badge work untouched; "Paused" was rejected because in this codebase it means one thing (ADR 0009) and raises an over-limit banner that would be false here. The Fix dialog's hidden variant adds the channel **id as selectable text** and an **Open in Discord** deep link (the row has no name to act on), plus a **Remove** action passed as an optional prop **only from the Channels tab** — Overview is read-only by contract and shares the dialog. **Zero bot changes**: the bot renders no Discord channel name (`/ap overview` emits `<#id>` mentions, which Discord resolves against the *viewer's* access), `GET /guild/:id/channels` returns ids only, and obfuscated channels keep `id` + `type`, so `pruneStaleChannels` still sees a complete list.
...
- **Filters is one rule at a time, picked by a channel select** — the tab's equivalent of the server switcher, carrying the channel name and its status line (`Publishes all` / `N conditions`). The channel rides the URL as `?channel=<id>`, written with `history.replaceState`: it selects a view inside one tool, not a separate resource, so a `/filters/:channelId` segment would buy a router round-trip and an RSC fetch per switch, a 404 story for a stale id, and a back entry per channel — with the query param Back still means "leave Filters", and the Channels tab's existing deep link keeps working unchanged. The select heads the form and Revert/Save close it, both outside the card; the action row stays **sticky** because a rule runs to 50 conditions and Save must stay in reach (`bottom-22` on mobile clears the fixed tab bar). The select is never locked: switching channels mid-edit drops the edit, and the `beforeunload` + anchor-intercept guard does not cover it — a deliberate trade for a control that stays usable. The editor is a real `<form>` (Save is the submit), and `useUnsavedChangesWarning` attaches `beforeunload` **only while dirty** — a permanently registered handler disqualifies the page from the back/forward cache — then, because that event never fires for App Router client navigation and the router exposes no blocker, intercepts same-origin anchor clicks in the capture phase with a native `confirm`. A condition is one grid row — field, operator, values, delete — placed explicitly so the desktop single line and the phone stack share one DOM order and therefore one tab order. Invalid values stay on screen flagged in place; the 50-condition and 25-value caps are **never displayed**, only enforced with an error on the attempt past them.
...
- **One fact, one place, per screen.** A channel row is a severity dot plus the name — a column of identical megaphones stated nothing the row did not; a serving row is labelled `Publishing` (or `Paused`) on both tabs — a Channels row's toggle says what the channel is set to, not whether it works — while a **broken** row carries the `Fix` control and no label, on either tab: the control names the state and acts on it, and the Overview card's headline already counts the broken channels. Disabled Channels rows stay bare. The filter count is Channels-only (`ChannelFilterPill`, icon + count, linked to the Filters tab) — Overview showed one too and it earned nothing on a read-only surface. So is the severity chrome: `ChannelStatusLabel icon` and the filled `ChannelFixButton` are Channels; Overview passes neither, because its status card already carries the severity; plan identity is stated once, in the server switcher.
